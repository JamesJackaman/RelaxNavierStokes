...
if __name__=="__main__":
    ...
    # Just run "successful" cases from before
    tdegrees = [1, 1, 1, 0, 0, 0, 1, 0, 0, 0]
    sdegrees = [1, 1, 1, 1, 1, 1, 2, 2, 2, 2]
    dts = [0.01, 0.005, 0.0025, 0.01, 0.005, 0.0025, 0.01, 0.01, 0.005, 0.0025]
    ...
    for Mref in Mrefs:
        for R in Rs:
            for tdegree, sdegree, dt in zip(tdegrees, sdegrees, dts):
                runnum = 0
                while runnum<5:
                    N = int(N_ * (dt_/dt))
                    print('Mref =', Mref)
