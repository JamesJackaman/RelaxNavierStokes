            ...
            for sdegree in sdegrees:
                for R in Rs:
                    ...
                    while runnum < 5:
                        ...
                        process = subprocess.Popen('mpiexec -n %s python lid_caller.py --stepper %s --N %s --dt %s --tdegree %s --sdegree %s --R %s --Mbase %s --Mref %s --tmpname %s' % (MPIProcesses, '--'+args.flags, N, dt, tdegree, sdegree, R, Mbase, Mref, tmpname),
